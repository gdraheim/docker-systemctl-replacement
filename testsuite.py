...
class DockerSystemctlReplacementTest(unittest.TestCase):
    ...
    def test_4030_simple_service_functions(self):
        ...
        self.rm_testdir()
        self.coverage()
    def test_4031_simple_service_functions_restart(self):
        """ check that we manage simple services in a root env
            with commands like start, restart, stop, etc.
            With a variant on ExecRestart. """
        testname = self.testname()
        testdir = self.testdir()
        user = self.user()
        root = self.root(testdir)
        ...
        text_file(os_path(testdir, "zzz.service"),"""
            ...
            ExecStop=/usr/bin/kill -3 $MAINPID
            ExecStopPost=sleep 2
            ExecRestartPre=echo restaating $MAINPID
            ExecRestart=/usr/bin/kill -12 $MAINPID
            ExecRestartPost=sleep 1
            KillSignal=SIGQUIT
            [Install]
            WantedBy=multi-user.target
            """.format(**locals()))
        shell_file(os_path(bindir, testscript),"""
            ...
            {end}
            restart () {begin}
              date +%T,restaating >> {logfile}
              sleep 1
              date +%T,restaated >> {logfile}
            {end}
            trap "stops" 3
            trap "reload" 10
            trap "restart" 12
        ...
        top2 = top
        #
        logg.info("-- and we check that there is NO new PID in this variant")
        def find_pids(ps_output, command):
            pids = []
            for line in ps_output.split("\n"):
                if command not in line: continue
        ...
        self.assertTrue(len(ps1), 1)
        self.assertTrue(len(ps2), 1)
        self.assertEqual(ps1[0], ps2[0])
        #
        # inspect the service's log
        log = lines(open(logfile))
        logg.info("LOG\n %s", "\n ".join(log))
        self.assertTrue(greps(log, "restaating"))
        self.assertFalse(greps(log, "reload"))
        os.remove(logfile)
        #
        ...
        top7 = top
        #
        logg.info("-- and we check that there is no PID for this variant")
        ps7 = find_pids(top7, testsleep)
        logg.info("found PIDs %s and %s", ps6, ps7)
        self.assertTrue(len(ps6), 1)
        self.assertTrue(len(ps7), 1)
        self.assertEqual(ps6[0], ps7[0])
        #
        # inspect the service's log
        log = lines(open(logfile))
        logg.info("LOG\n %s", "\n ".join(log))
    ...
    def test_4032_forking_service_functions(self):
        ...
        self.rm_testdir()
        self.coverage()
    def test_4033_forking_service_functions_with_restart(self):
        """ check that we manage forking services in a root env
            with basic run-service commands: start, stop, restart,
            reload, try-restart, reload-or-restart, kill and
            reload-or-try-restart. This variant has signals for
            restart/reload actions instead of running stop/start"""
        testname = self.testname()
        testdir = self.testdir()
        user = self.user()
        root = self.root(testdir)
        ...
        text_file(os_path(testdir, "zzz.service"),"""
            ...
            PIDFile={root}/var/run/zzz.init.pid
            ExecStart={root}/usr/bin/zzz.init start
            ExecRestart={root}/usr/bin/zzz.init restart
        ...
        self.assertTrue(greps(log, "starting"))
        self.assertFalse(greps(log, "stopped"))
        self.assertFalse(greps(log, "restaating"))
        self.assertFalse(greps(log, "reloading"))
        os.remove(logfile)
        #
        logg.info("== 'stop' shall stop a service that is-active")
        stop_service = "{systemctl} stop zzz.service -vv"
        ...
        self.assertFalse(greps(log, "stopped"))
        self.assertFalse(greps(log, "restaating"))
        self.assertTrue(greps(log, "reloading"))
        os.remove(logfile)
        #
        logg.info("== 'reload-or-restart' will reload a service that is-active (if ExecReload)")        
        restart_service = "{systemctl} reload-or-restart zzz.service -vv"
        sh____(restart_service.format(**locals()))
        top_recent = "ps -eo etime,pid,ppid,args --sort etime,pid | grep '^ *0[0123]:[^ :]* '"
        top = output(top_recent.format(**locals()))
    ...
    def test_4035_notify_service_functions(self):
        ...
        self.rm_testdir()
        self.coverage()
    def test_4036_notify_service_functions_with_reload_and_restart(self):
        """ check that we manage notify services in a root env
            with basic run-service commands: start, stop, restart,
            reload, try-restart, reload-or-restart, kill and
            reload-or-try-restart. (with ExecReload and ExecRestart)"""
        if not os.path.exists("/usr/bin/socat"):
            self.skipTest("missing /usr/bin/socat")
    ...
    def test_4037_oneshot_service_functions(self):
        ...
        self.rm_testdir()
        self.coverage()
    def test_4038_oneshot_service_functions_and_restart(self):
        """ check that we manage oneshot services in a root env
            with basic run-service commands: start, stop, restart,
            reload, try-restart, reload-or-restart, kill and
            reload-or-try-restart. This variant includes ExecRestart lines."""
        if not os.path.exists("/usr/bin/socat"):
            self.skipTest("missing /usr/bin/socat")
        testname = self.testname()
        testdir = self.testdir()
        ...
        text_file(os_path(testdir, "zzz.service"),"""
            ...
            ExecStartPre={bindir}/backup {root}/var/tmp/test.1 {root}/var/tmp/test.2
            ExecStart=/usr/bin/touch {root}/var/tmp/test.1
            ExecRestart=/usr/bin/touch {root}/var/tmp/test.3
            ExecStop=/usr/bin/rm {root}/var/tmp/test.1
            ExecStopPost=/usr/bin/rm -f {root}/var/tmp/test.2
            ExecStopPost=/usr/bin/rm -f {root}/var/tmp/test.3
            [Install]
            WantedBy=multi-user.target
            """.format(**locals()))
        shell_file(os_path(testdir, "backup"), """
        ...
        self.assertTrue(os.path.exists(os_path(root, "/var/tmp/test.1")))
        self.assertFalse(os.path.exists(os_path(root, "/var/tmp/test.2")))
        self.assertTrue(os.path.exists(os_path(root, "/var/tmp/test.3")))
        #
        logg.info("== 'reload' will NOT restart a service that is-active")        
        restart_service = "{systemctl} reload zzz.service -vv"
        sh____(restart_service.format(**locals()))
        ...
        self.assertTrue(os.path.exists(os_path(root, "/var/tmp/test.3")))
        #
        os.remove(os_path(root, "/var/tmp/test.3"))
        self.assertFalse(os.path.exists(os_path(root, "/var/tmp/test.3")))
        #
        logg.info("== 'reload-or-restart' will restart a service that is-active")        
        restart_service = "{systemctl} reload-or-restart zzz.service -vv"
        ...
        self.assertTrue(os.path.exists(os_path(root, "/var/tmp/test.3")))
        #
        logg.info("== 'stop' will bring it back to 'inactive'")        
        restart_service = "{systemctl} stop zzz.service -vv"
        sh____(restart_service.format(**locals()))
        act = output(is_active.format(**locals()))
        self.assertEqual(act.strip(), "inactive")
